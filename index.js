/*
]=====> ALIA ID <=====[ ]=====> YT ALIA ID <=====[ ]=====> 83895134139 <=====[
*/
// ANAK ANJING PASTI YANG ATAS DI UBAH
// NGOTAK KONSOL BABI NGETOD
// NUMPANG NAMA TIDAK MEMBUAT MU PRO TOLOL
const {
   WAConnection,
   MessageType,
   Presence,
...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:ALIA ID\n' // GANTI NAMA LU
            + 'ORG:OWNER BOTZ;\n'
            + 'TEL;type=CELL;type=VOICE;waid=6283895134139:+62 838-9513-4139\n' // GANTI NOMOR LU
            + 'END:VCARD'
...
console.log(banner.string)
   ...
   qrcode.generate(qr, { small: true })
	console.log(color('[','white'), color('!','red'), color(']','white'), color(' SUBSCRIBE YT DARK BOT OFFICIAL HD CHANNEL'))
})
	...
	baby.on('connecting', () => {
		start('2', 'Alia Connecting...')
	})
	baby.on('open', () => {
		success('2', 'Alia Connected')
	})
...
// ANTI LINK GRUP
		        ...
		        reply(`Woyy ${sender.split("@")[0]} Gak Boleh Share Link Group Kontol😡`)
		        setTimeout( () => {
			        baby.groupRemove(from, [kic]).catch((e)=>{reply(`BOT HARUS JADI ADMIN`)})
		        }, 3000)
		        setTimeout( () => {
			        baby.updatePresence(from, Presence.composing)
			        reply("HEADSHOT KEPALA KAU")
		        }, 2000)
		        setTimeout( () => {
			        baby.updatePresence(from, Presence.composing)
			        reply("Bismillah")
...
*/				
            	case 'mining':
                      ...
                      if (!isEventon) return reply(`maaf ${pushname} event mining tidak di aktifkan sama owner Alia`)
                      if (isOwner) {
                      const one = 999999999
                      addLevelingXp(sender, one)
                      addLevelingLevel(sender, 99)
                      reply(`karena Alia baik Bot memberikan ${one}Xp >_<`)
                      }else{
            ...
            case 'creator':
                  ...
                  baby.sendMessage(from, 'Tuh Nomor Ownerku Jangan Spam Atau Ku Block Kamu:)',MessageType.text, { quoted: mek} )
					break
case 'hartatahta':
if (isBanned) return reply(nad.baned())
if (!isRegistered) return reply(nad.noregis())
...
*/
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							baby.sendMessage(_.jid, buff, image, {caption: `*「 BOT BROADCAST 」*\n\n${body.slice(4)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
				...
				case 'bcgc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						buff = await baby.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							baby.sendMessage(_.jid, buff, image, {caption: `*「 BROADCAST GROUP 」*\n\n➸ Dari Grup : ${groupName}\n➸ Pengirim : wa.me/${(sender.split('@')[0])}\n➸ Pesan : ${body.slice(6)}`})
						}
						reply('*「 SUKSES BOSKU 」*')
					} else {
						for (let _ of anu) {
